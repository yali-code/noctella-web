 ...
 * staged photo and is always sent explicitly on Finalize - never relies on
 * backend omission fallback. Reselects the first photo if the previously
 * selected one is removed by a refresh.
 */
export function ApplyAndFinalizeSection({ intakeId, intake, photos, proposal, onIntakeChanged, onPhotosReload }: Props) {
  ...
  async function handleSaveAsDraft() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Failed to save as draft");
    } finally {
      setSaving(false);
    }
  }
  ...
  const canSaveAsDraft = intake.status === AiProductIntakeStatus.Open;
  ...
  return (
    <div className="noctella-panel" style={{ padding: 20, marginBottom: 20 }}>
      <h3 style={{ marginTop: 0 }}>Apply and Finalize</h3>
      ...
      {canSaveAsDraft && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gap: 8, maxWidth: 360 }}>
            <input placeholder="SKU" value={sku} onChange={(e) => setSku(e.target.value)} style={inputStyle} />
          ...
          {confirmSave ? (
            <div style={{ marginTop: 8 }}>
              <p style={{ fontSize: 12 }}>Create this Product as Draft?</p>
              <button onClick={handleSaveAsDraft} disabled={saving || !proposal || !sku || !categoryId || !priceEur} style={primaryButtonStyle}>
                {saving ? "Saving..." : "Confirm Save as Draft"}
              </button>{" "}
              <button onClick={() => setConfirmSave(false)} style={secondaryButtonStyle}>
                Cancel
              </button>
            </div>
          ) : (
            <button onClick={() => setConfirmSave(true)} disabled={!proposal} style={{ ...primaryButtonStyle, marginTop: 8 }}>
              Save as Draft
            </button>
          )}
        </div>
