...
describe("purchases list proxy route (Sprint 57B)", () => {
  it("forwards the query string and injects the ERP key server-side", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/purchases?status=Draft&supplierId=s1"));
    expect(mockFetch).toHaveBeenCalledWith(
      "http://backend.internal:4000/api/erp/purchases?status=Draft&supplierId=s1",
  ...
  it("forwards non-ok upstream statuses", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/purchases"));
