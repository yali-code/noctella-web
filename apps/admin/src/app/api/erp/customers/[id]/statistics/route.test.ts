...
describe("customer statistics proxy route (Sprint 61B)", () => {
  it("forwards the customer id and injects the ERP key server-side", async () => {
    const mockFetch = vi.spyOn(global, "fetch").mockResolvedValue(new Response(JSON.stringify({ lifetimeValue: null }), { status: 200, headers: { "content-type": "application/json" } }));
    const res = await GET(new Request("http://admin.local/api/erp/customers/c1/statistics"), { params: { id: "c1" } });
    expect(mockFetch).toHaveBeenCalledWith(
      "http://backend.internal:4000/api/erp/customers/c1/statistics",
