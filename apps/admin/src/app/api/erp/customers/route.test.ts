...
describe("customers list proxy route (Sprint 61B)", () => {
  it("forwards the query string and injects the ERP key server-side", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/customers?search=ada"));
    expect(mockFetch).toHaveBeenCalledWith(
      "http://backend.internal:4000/api/erp/customers?search=ada",
  ...
  it("forwards non-ok upstream statuses", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/customers"));
