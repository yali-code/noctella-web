...
describe("picking list proxy route (Sprint 59B)", () => {
  it("injects the ERP key server-side and forwards the query string", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/picking?orderId=o1"));
    expect(mockFetch).toHaveBeenCalledWith(
      "http://backend.internal:4000/api/erp/picking?orderId=o1",
  ...
  it("fails closed without exposing the key when ERP_INTEGRATION_KEY is unset", async () => {
    ...
    const res = await GET(new Request("http://admin.local/api/erp/picking"));
