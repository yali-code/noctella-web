...
const request = () => new Request("https://admin.example.test/api/erp/products/p/purchase-history", { headers: { "X-Noctella-ERP-Key": "untrusted", Cookie: "untrusted" } });
describe("product purchase history ERP proxy", () => {
  it("uses encoded fixed paths and server-only credentials without forwarding browser headers", async () => {
    const body = { productId: "p", items: [] };
    ...
    expect(fetchSpy).toHaveBeenCalledExactlyOnceWith("https://backend.example.test/api/erp/products/p%2Fwith%20space/purchase-history", {
