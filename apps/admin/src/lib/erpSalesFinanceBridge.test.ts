...
import { buildSalesFinanceQuery, cancelInvoice, createInvoiceDraft, invoiceActionEligibility, invoiceCommand, issueInvoice, markInvoicePaid, mapFinanceSummary, mapInvoice, mapRefundSummary, mapReversalSummary, mapSale, maskCustomer, redactSafeError } from "./erpSalesFinanceBridge";
...
describe("issueInvoice", () => {
  ...
  it("posts to the same-origin Admin proxy path with a fresh idempotencyKey and empty payload", async () => {
    const mockFetch = vi.spyOn(global, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ id: "inv-1", status: "Issued" }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    );
    ...
    expect(mockFetch).toHaveBeenCalledTimes(1);
    const [url, init] = mockFetch.mock.calls[0];
    expect(url).toBe("/api/erp/commands/invoices/inv-1/issue");
    expect(init?.method).toBe("POST");
    const body = JSON.parse(init?.body as string);
    expect(typeof body.idempotencyKey).toBe("string");
    expect(body.idempotencyKey.length).toBeGreaterThan(0);
...
describe("cancelInvoice", () => {
  ...
  it("posts to the same-origin Admin proxy path with a fresh idempotencyKey and empty payload", async () => {
    const mockFetch = vi.spyOn(global, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ id: "inv-1", status: "Cancelled" }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    );
    ...
    expect(mockFetch).toHaveBeenCalledTimes(1);
    const [url, init] = mockFetch.mock.calls[0];
    expect(url).toBe("/api/erp/commands/invoices/inv-1/cancel");
    expect(init?.method).toBe("POST");
    const body = JSON.parse(init?.body as string);
    expect(typeof body.idempotencyKey).toBe("string");
    expect(body.idempotencyKey.length).toBeGreaterThan(0);
  ...
  it("throws an ApiError with the backend status on failure", async () => {
    ...
    await expect(cancelInvoice("inv-1")).rejects.toMatchObject({ status: 404 });
    await expect(cancelInvoice("inv-1")).rejects.toBeInstanceOf(ApiError);
  });
});
