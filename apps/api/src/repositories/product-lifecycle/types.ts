...
export interface ProductLifecycleRepository {
  ...
  getLatestByAction(productId: string, action: "pause" | "relist"): Promise<ProductLifecycleOperation | undefined>;
  ...
  clearSalePause(productId: string, expectedPausedAt: string, updatedAt: string): Promise<boolean>;
}
