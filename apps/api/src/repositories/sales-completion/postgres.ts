...
export function createPostgresSalesCompletionTransactionRepository(db: any): SalesCompletionTransactionRepository { return Object.freeze({ async commit(input: SalesCompletionCommitInput) {
  ...
  const finalized = await db.update(schema.orders).set({ status: "Completed", updatedAt: new Date(input.finalUpdatedAt) }).where(eq(schema.orders.id, input.snapshot.saleId)).returning({ id: schema.orders.id });
  if (finalized.length !== 1) throw new SalesCompletionCoordinationError("sale_final_state", "not_found");
  return result;
} }); }
