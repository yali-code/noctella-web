export type ProductReadListQuery = { page?: number; pageSize?: number; limit?: number; offset?: number; search?: string; status?: string; type?: string; categoryId?: string; collectionId?: string; categorySlug?: string; collectionSlug?: string; published?: boolean; isFeatured?: boolean; showInArchiveAfterSale?: boolean; excludeId?: string; updatedSince?: string; sort?: string };
...
/**
 * Sprint 139: the Pending Publish queue's minimal row contract. Membership authority is Warehouse
 * Stock Acceptance provenance (see listPendingPublish below) - not a persisted status - so this is
 * deliberately its own small projection rather than reusing ProductListProjection, which carries no
 * concept of Stock Acceptance at all. stockAcceptedAt is the authoritative ai_product_intakes.appliedAt
 * value for the intake whose resultProductId resolved to this Product, exactly as approved.
 */
export type ProductPendingPublishProjection = { id: string; title: string; stockAcceptedAt: string };
