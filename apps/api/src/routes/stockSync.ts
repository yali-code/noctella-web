...
const router = Router();
router.get("/audit", requirePermission("marketplace.view"), async (req, res, next) => { try { res.json({ items: await db.select().from(stockSyncAudit).orderBy(desc(stockSyncAudit.createdAt)).limit(Number(req.query.pageSize ?? 50)) }); } catch (e) { next(e); } });
router.get("/conflicts", requirePermission("marketplace.view"), async (req, res, next) => { try { const filters = [req.query.status && eq(stockSyncConflicts.status, String(req.query.status)), req.query.channel && eq(stockSyncConflicts.channel, String(req.query.channel)), req.query.productId && eq(stockSyncConflicts.productId, String(req.query.productId))].filter(Boolean) as any[]; res.json({ items: await db.select().from(stockSyncConflicts).where(filters.length ? and(...filters) : undefined).orderBy(desc(stockSyncConflicts.createdAt)).limit(Number(req.query.pageSize ?? 50)) }); } catch (e) { next(e); } });
router.get("/conflicts/:id", requirePermission("marketplace.view"), async (req, res, next) => { try { const [item] = await db.select().from(stockSyncConflicts).where(eq(stockSyncConflicts.id, req.params.id)); res.json(item); } catch (e) { next(e); } });
router.post("/conflicts/:id/resolve", requirePermission("marketplace.manage"), async (req, res, next) => { try { res.json(await resolveStockSyncConflict(db, req.params.id, String(req.body?.action ?? "MarkResolved"))); } catch (e) { next(e); } });
router.get("/status", requirePermission("marketplace.view"), async (_req, res, next) => { try { res.json(await stockSyncSummary(db)); } catch (e) { next(e); } });
