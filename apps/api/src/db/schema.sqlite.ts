...
export const payments = sqliteTable("payments", { id: text("id").primaryKey(), orderId: text("order_id"), provider: text("provider").notNull(), providerReference: text("provider_reference"), status: text("status").notNull(), amount: real("amount").notNull(), currency: text("currency").notNull().default("EUR"), idempotencyKey: text("idempotency_key").notNull().unique(), safeMetadata: text("safe_metadata"), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table)=>[index("idx_payments_order").on(table.orderId), index("idx_payments_provider_reference").on(table.provider, table.providerReference)]);
