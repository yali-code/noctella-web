...
}, (table) => [uniqueIndex("idx_marketplace_connections_channel_account").on(table.channel, table.accountLabel), index("idx_marketplace_connections_channel").on(table.channel)]);
export const instagramPublishAttempts = sqliteTable("instagram_publish_attempts", {
  id: text("id").primaryKey(),
...
export const invoices = sqliteTable("invoices", { id: text("id").primaryKey(), erpReferenceId: text("erp_reference_id"), orderId: text("order_id").notNull(), customerId: text("customer_id"), invoiceNumber: text("invoice_number"), invoiceType: text("invoice_type").notNull(), status: text("status").notNull(), currency: text("currency").notNull().default("EUR"), calculationMode: text("calculation_mode").notNull().default("Automatic"), taxTreatment: text("tax_treatment").notNull().default("StandardVAT"), pricesIncludeVat: integer("prices_include_vat", { mode: "boolean" }).notNull().default(false), issuedAt: text("issued_at"), dueAt: text("due_at"), paidAt: text("paid_at"), cancelledAt: text("cancelled_at"), sellerSnapshot: text("seller_snapshot").notNull(), customerSnapshot: text("customer_snapshot").notNull(), billingAddressSnapshot: text("billing_address_snapshot"), shippingAddressSnapshot: text("shipping_address_snapshot"), subtotal: real("subtotal").notNull(), shippingAmount: real("shipping_amount").notNull().default(0), shippingVatRate: real("shipping_vat_rate").notNull().default(0), shippingVatAmount: real("shipping_vat_amount").notNull().default(0), discountAmount: real("discount_amount").notNull().default(0), taxVatAmount: real("tax_vat_amount").notNull().default(0), totalAmount: real("total_amount").notNull(), notes: text("notes"), invoiceFooter: text("invoice_footer"), sourceSnapshot: text("source_snapshot").notNull(),
  // Sprint 79 correction: nullable, not order_id itself — see migrate.ts's ensureInvoiceAccountingColumns
  // for why a direct unique index on order_id (filtered to SalesInvoice) is unsafe against
  // historical duplicate SalesInvoice rows that were legal before this sprint. Only ever NULL for
  // a pre-existing ambiguous duplicate group; every new SalesInvoice always sets this to its own order_id.
  salesInvoiceOrderKey: text("sales_invoice_order_key"),
  createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [uniqueIndex("idx_invoices_erp_reference_unique").on(table.erpReferenceId), uniqueIndex("idx_invoices_number_unique").on(table.invoiceNumber), index("idx_invoices_order").on(table.orderId), index("idx_invoices_customer").on(table.customerId), index("idx_invoices_status").on(table.status), index("idx_invoices_type").on(table.invoiceType), index("idx_invoices_dates").on(table.issuedAt, table.createdAt)]);
export const invoiceLines = sqliteTable("invoice_lines", { id: text("id").primaryKey(), invoiceId: text("invoice_id").notNull(), orderItemId: text("order_item_id"), productId: text("product_id"), skuSnapshot: text("sku_snapshot"), titleSnapshot: text("title_snapshot").notNull(), quantity: integer("quantity").notNull(), unitPrice: real("unit_price").notNull(), discountAmount: real("discount_amount").notNull().default(0), vatRate: real("vat_rate").notNull().default(0), taxVatAmount: real("tax_vat_amount").notNull().default(0), lineTotal: real("line_total").notNull(), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [index("idx_invoice_lines_invoice").on(table.invoiceId), index("idx_invoice_lines_order_item").on(table.orderItemId), index("idx_invoice_lines_product").on(table.productId)]);
...
export const customerAddresses = sqliteTable("customer_addresses", { id: text("id").primaryKey(), customerId: text("customer_id").notNull(), type: text("type").notNull(), name: text("name"), line1: text("line1"), line2: text("line2"), city: text("city"), region: text("region"), postalCode: text("postal_code"), country: text("country"), countryCode: text("country_code"), fingerprint: text("fingerprint"), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [index("idx_customer_addresses_customer").on(table.customerId), index("idx_customer_addresses_fingerprint").on(table.fingerprint)]);
export const customerAccounts = sqliteTable("customer_accounts", { id: text("id").primaryKey(), customerId: text("customer_id").notNull().unique(), normalizedEmail: text("normalized_email").notNull().unique(), passwordHash: text("password_hash"), emailVerifiedAt: text("email_verified_at"), status: text("status").notNull().default("active"), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull() });
export const customerSessions = sqliteTable("customer_sessions", { id: text("id").primaryKey(), accountId: text("account_id").notNull(), tokenHash: text("token_hash").notNull().unique(), expiresAt: text("expires_at").notNull(), revokedAt: text("revoked_at"), createdAt: text("created_at").notNull() }, (table) => [index("idx_customer_sessions_account").on(table.accountId), index("idx_customer_sessions_expiry").on(table.expiresAt)]);
export const customerSecurityTokens = sqliteTable("customer_security_tokens", { id: text("id").primaryKey(), accountId: text("account_id").notNull(), purpose: text("purpose").notNull(), tokenHash: text("token_hash").notNull().unique(), expiresAt: text("expires_at").notNull(), consumedAt: text("consumed_at"), createdAt: text("created_at").notNull() }, (table) => [index("idx_customer_security_tokens_account").on(table.accountId, table.purpose), index("idx_customer_security_tokens_expiry").on(table.expiresAt)]);
export const customerAuthProviders = sqliteTable("customer_auth_providers", { id: text("id").primaryKey(), accountId: text("account_id").notNull(), provider: text("provider").notNull(), subject: text("subject").notNull(), createdAt: text("created_at").notNull() }, (table) => [uniqueIndex("idx_customer_auth_providers_subject").on(table.provider, table.subject), uniqueIndex("idx_customer_auth_providers_account").on(table.accountId, table.provider)]);
export const customerOAuthAttempts = sqliteTable("customer_oauth_attempts", { id: text("id").primaryKey(), stateHash: text("state_hash").notNull().unique(), nonce: text("nonce").notNull(), codeVerifier: text("code_verifier").notNull(), expiresAt: text("expires_at").notNull(), consumedAt: text("consumed_at"), createdAt: text("created_at").notNull() }, (table) => [index("idx_customer_oauth_attempts_expiry").on(table.expiresAt)]);
 ...
 * Category, Collections, SEO keywords, and Etsy listing tags (see Sprint 139/140 Discovery).
 * `key` is the canonicalized machine key (see services/marketingTags.ts's canonicalizeMarketingTagKey -
 * deliberately NOT the shared validation/common.ts slugify(), which hyphenates apostrophes rather
 * than stripping them); `label` is the admin-facing friendly text. No FK constraint on
 * product_marketing_tags, matching this schema's established convention (e.g.
 * ai_product_intakes.resultProductId).
 */
export const marketingTags = sqliteTable("marketing_tags", { id: text("id").primaryKey(), key: text("key").notNull(), label: text("label").notNull(), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [uniqueIndex("idx_marketing_tags_key_unique").on(table.key)]);
export const productMarketingTags = sqliteTable("product_marketing_tags", { id: text("id").primaryKey(), productId: text("product_id").notNull(), marketingTagId: text("marketing_tag_id").notNull(), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [uniqueIndex("idx_product_marketing_tags_unique").on(table.productId, table.marketingTagId)]);
...
export const adminUsers = sqliteTable("admin_users", { id: text("id").primaryKey(), email: text("email").notNull().unique(), passwordHash: text("password_hash").notNull(), role: text("role").notNull(), status: text("status").notNull().default("active"), sessionVersion: integer("session_version").notNull().default(1), lastLoginAt: text("last_login_at"), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) });
export const adminSessions = sqliteTable("admin_sessions", { id: text("id").primaryKey(), tokenHash: text("token_hash").notNull().unique(), adminUserId: text("admin_user_id").notNull(), sessionVersion: integer("session_version").notNull(), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), lastSeenAt: text("last_seen_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), expiresAt: text("expires_at").notNull() }, (table) => [index("idx_admin_sessions_admin_user").on(table.adminUserId)]);
export const adminAuthEvents = sqliteTable("admin_auth_events", { id: text("id").primaryKey(), adminUserId: text("admin_user_id"), email: text("email").notNull(), eventType: text("event_type").notNull(), ipAddress: text("ip_address"), userAgent: text("user_agent"), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table) => [index("idx_admin_auth_events_email_created").on(table.email, table.createdAt), index("idx_admin_auth_events_admin_user").on(table.adminUserId)]);
...
export const payments = sqliteTable("payments", { id: text("id").primaryKey(), orderId: text("order_id"), provider: text("provider").notNull(), providerReference: text("provider_reference"), providerTransactionReference: text("provider_transaction_reference"), status: text("status").notNull(), amount: real("amount").notNull(), expectedAmountCents: integer("expected_amount_cents"), currency: text("currency").notNull().default("EUR"), idempotencyKey: text("idempotency_key").notNull().unique(), checkoutSnapshot: text("checkout_snapshot"), safeMetadata: text("safe_metadata"), createdAt: text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt: text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table)=>[index("idx_payments_order").on(table.orderId), uniqueIndex("idx_payments_provider_reference_unique").on(table.provider, table.providerReference).where(sql`provider_reference IS NOT NULL`), uniqueIndex("idx_payments_provider_transaction_unique").on(table.provider, table.providerTransactionReference).where(sql`provider_transaction_reference IS NOT NULL`), uniqueIndex("idx_payments_order_unique").on(table.orderId).where(sql`order_id IS NOT NULL`)]);
export const paymentEvents = sqliteTable("payment_events", { id:text("id").primaryKey(), provider:text("provider").notNull(), providerEventId:text("provider_event_id").notNull(), eventType:text("event_type").notNull(), paymentId:text("payment_id"), status:text("status").notNull(), resultClassification:text("result_classification"), errorCode:text("error_code"), createdAt:text("created_at").notNull().default(sql`(CURRENT_TIMESTAMP)`), updatedAt:text("updated_at").notNull().default(sql`(CURRENT_TIMESTAMP)`) }, (table)=>[uniqueIndex("idx_payment_events_provider_event_unique").on(table.provider,table.providerEventId),index("idx_payment_events_payment").on(table.paymentId)]);
