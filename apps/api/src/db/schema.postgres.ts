...
export const externalListings = pgTable("external_listings", {
  ...
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().default(sql`now()`),
}, (table) => [uniqueIndex("idx_external_listings_channel_external").on(table.channel, table.externalListingId)]);
...
export const saleCompletionExecutions = pgTable("sale_completion_executions", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().default(sql`now()`),
}, (table) => [uniqueIndex("idx_sale_completion_executions_sale_unique").on(table.saleId)]);
