...
app.post("/api/marketplaces/stock-sync/all", requirePermission("marketplace.manage"), async (req, res, next) => { try { res.json({ jobs: await Promise.all(STOCK_SYNC_CHANNELS.map((channel) => enqueueChannelStockSync(db, channel))) }); } catch (e) { next(e); } });
