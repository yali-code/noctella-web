...
export const stockAcceptanceSchema = z
  .object({
    ...
    purchaseSource: z.string().trim().min(1).optional(),
    auctionHouse: z.string().trim().min(1).optional(),
    invoiceReferenceNumber: z.string().trim().min(1).optional(),
    provenance: z.string().trim().min(1).optional(),
    previousOwner: z.string().trim().min(1).optional(),
