...
abstract class HttpAdapter implements MarketplaceAdapter {
  ...
  // Sprint 77: no fallback secret. A missing, empty, or whitespace-only *_WEBHOOK_SECRET is
  // treated as unconfigured and fails closed (false) - externally indistinguishable from an
  // invalid signature, since processWebhook() rejects both with the exact same BadRequestError.
  verifyWebhookSignature(rawBody: Buffer, headers: Record<string, string | string[] | undefined>) { const secret = this.env[`${this.prefix}_WEBHOOK_SECRET`]; if (!secret || !secret.trim()) return false; const sig = String(headers[`${this.prefix.toLowerCase()}-signature`] ?? headers["x-marketplace-signature"] ?? ""); const expected = crypto.createHmac("sha256", secret).update(rawBody).digest("hex"); return sig === expected; }
