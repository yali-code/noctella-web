...
import { createGetInventoryUseCase } from "../application/inventory";
...
/** Sprint 49B: same policy value as Sprint 46B/47B/48B, kept local so this file's command lifecycle stays self-contained. */
const ERP_INVENTORY_COMMAND_ACCEPTED_STALE_MS = 60_000;
...
async function execution(db: DbClient, clientId: string, env: Envelope) {
  ...
  if (existing) {
    ...
    if (existing.status === "Succeeded") return { existing, sum };
    if (existing.status === "Accepted") {
      const isStale = Date.now() - new Date(existing.createdAt).getTime() > ERP_INVENTORY_COMMAND_ACCEPTED_STALE_MS;
    ...
    await db.update(erpCommandExecutions).set({ status: "Accepted", safeErrorCode: null, completedAt: null, createdAt: new Date().toISOString() }).where(and(eq(erpCommandExecutions.clientId, clientId), eq(erpCommandExecutions.idempotencyKey, env.idempotencyKey)));
  ...
  try {
    ...
    if (String(error?.message ?? error).includes("UNIQUE constraint failed")) return execution(db, clientId, env);
    throw error;
  }
  return { existing: null, sum };
}
...
async function fail(db: DbClient, clientId: string, env: Envelope, entityId: string | null, error: unknown) { await complete(db, clientId, env, "Failed", entityId, undefined, safeErrorCodeFor(error)); }
...
export async function executeStockAdjustment(db: DbClient, clientId: string, productId: string, env: Envelope) {
  ...
  let meta: any;
  try {
    const delta=Number(p.quantityDelta); if(!Number.isInteger(delta) || delta===0) throw new BadRequestError("Non-zero integer quantityDelta is required"); if(!p.reason && !p.note) throw new BadRequestError("reason or note is required");
    const movement = await applyStockMovement(db, { productId, type: StockMovementType.ManualAdjustment, quantityDelta: delta, note: String(p.reason ?? p.note), idempotencyKey: `erp:${clientId}:${env.idempotencyKey}` });
    meta={ productId, previousQuantity: movement.stockBefore, delta, newQuantity: movement.stockAfter, movementId: movement.id };
  } catch (error) {
    await fail(db, clientId, env, productId, error);
    throw error;
  }
  await complete(db, clientId, env, "Succeeded", productId, meta); return { status:"Succeeded", ...meta };
}
