...
import { orderItems, orders, productImages, products, stockMovements } from "../db/schema";
import { BadRequestError, NotFoundError } from "./errors";
import type { CreateOrderInput, OrderListQuery, UpdateOrderStatusInput } from "../validation/order";
import { createStockMovementInTransaction } from "./stockMovements";
...
function saleMovementExists(
  ...
  const [existing] = tx
    ...
    .where(
      and(
        eq(stockMovements.type, StockMovementType.Sale),
...
export async function updateOrderStatus(
  ...
  db.transaction((tx) => {
    const [order] = tx.select().from(orders).where(eq(orders.id, id)).all();
    ...
    if (input.status === OrderStatus.Processing) {
      for (const item of items) {
        if (!saleMovementExists(tx, id, item.productId)) {
          createStockMovementInTransaction(
            tx,
            {
    ...
    tx.update(orders).set({ status: input.status, updatedAt: now }).where(eq(orders.id, id)).run();
  });
