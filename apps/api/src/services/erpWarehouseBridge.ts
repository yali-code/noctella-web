...
export const shipmentReadyQueue=(db:DbClient)=>all(db,sql`SELECT o.id orderId,o.order_number orderNumber,p.id packingTaskId,p.shipment_id shipmentId,substr(o.guest_email,1,2)||'***' customerMaskedSummary,NULL carrier,NULL trackingAvailability,p.status packingStatus,p.package_count packageCount,p.total_weight totalWeight,p.completed_at readyAt FROM packing_tasks p JOIN orders o ON o.id=p.order_id WHERE p.status='ReadyForShipment' AND p.shipment_id IS NULL ORDER BY p.completed_at DESC`);
