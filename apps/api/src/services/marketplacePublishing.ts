...
import { getMarketplaceAdapter, type MarketplaceAdapter, type MarketplacePublishPayload } from "./marketplaceAdapters";
...
import { createWooCommerceFetchTransport, type WooCommerceTransport } from "../integrations/woocommerce/connectionClient";
import { CanonicalWooCommercePublishAdapter, type WooCommercePublishAdapter } from "../integrations/woocommerce/publishingAdapter";
...
function conn(row: PublishingRow): MarketplaceConnection { const status=row.channel===PublishChannel.WooCommerce&&row.status==="verified"?MarketplaceConnectionStatus.Connected:row.status as MarketplaceConnectionStatus;return { id: row.id, channel: row.channel as PublishChannel, accountLabel: row.accountLabel, externalAccountId: row.externalAccountId ?? undefined, tokenExpiresAt: publishingTimestampToIso(row.tokenExpiresAt), scopes: parse(row.scopes, []), status, lastError: row.lastError ?? undefined, createdAt: publishingTimestampToIso(row.createdAt)!, updatedAt: publishingTimestampToIso(row.updatedAt)! }; }
...
function isPublishableConnection(row: PublishingRow, channel: PublishChannel) { return channel === PublishChannel.WooCommerce ? row.status === "verified" : row.status === MarketplaceConnectionStatus.Connected; }
function assertPublishableConnection(row: PublishingRow | undefined, channel: PublishChannel) { if (!row?.encryptedAccessToken || !isPublishableConnection(row, channel) || (channel === PublishChannel.WooCommerce && (!row.encryptedRefreshToken || !row.externalAccountId))) throw new BadRequestError("Marketplace connection is not connected"); if (row.tokenExpiresAt && Date.parse(String(row.tokenExpiresAt)) <= Date.now()) throw new BadRequestError("Marketplace connection is expired"); return row; }
async function active(db: DbClient, channel: PublishChannel) { const {q,marketplaceConnections}=persistence(db); const rows = (await q.select().from(marketplaceConnections).where(eq(marketplaceConnections.channel, channel))).filter((row:PublishingRow)=>isPublishableConnection(row,channel));const selected=channel===PublishChannel.WooCommerce?(rows.find((row:PublishingRow)=>row.accountLabel==="Default")??(rows.length===1?rows[0]:undefined)):rows[0];return assertPublishableConnection(selected, channel); }
...
async function executePublishCore(db: DbClient, productId: string, channel: PublishChannel, key?: string, adapter?: MarketplaceAdapter, options:InternalPublishOptions={}): Promise<PublishExecutionResult> {
  ...
  const validation = validatePublish(product, channel); if (!validation.valid) throw new BadRequestError("Publish validation failed");
  if (channel === PublishChannel.NoctellaWeb) return executeNoctellaWebPublish(db, productId, product, channel, key);
  const payload = buildPublishPayload(product, product.images, channel) as MarketplacePublishPayload;
  const idem = key ?? `${productId}:${channel}:${crypto.createHash("sha256").update(JSON.stringify(payload)).digest("hex")}`;
  const [existing] = await q.select().from(publishJobs).where(eq(publishJobs.idempotencyKey, idem));
...
async function retryPublishJobCore(db: DbClient, idv: string, adapter?: MarketplaceAdapter, exactConnectionId?:string, wooTransport?:WooCommerceTransport) {
...
function persistedJsonValue(value:unknown){const encoded=JSON.stringify(value);return encoded===undefined?undefined:JSON.parse(encoded);}
function persistedJsonEqual(left:unknown,right:unknown){return isDeepStrictEqual(persistedJsonValue(left),persistedJsonValue(right));}
export async function retryControlledRelistPublishJob(db:DbClient,idv:string,connectionId:string,adapter?:MarketplaceAdapter){const{job:j}=await getPublishJob(db,idv),product=await getProductById(db,j.productId),validation=validatePublish(product,j.channel);if(!validation.valid)throw new ConflictError("Product data changed since the relist attempt; manual reconciliation is required.");const current=buildPublishPayload(product,product.images,j.channel);if(!persistedJsonEqual(current,j.payloadSnapshot))throw new ConflictError("Product data changed since the relist attempt; manual reconciliation is required.");return retryPublishJobCore(db,idv,adapter,connectionId);}
