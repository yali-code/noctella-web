...
export class ProductPhotoPromotionHandler implements OutboxHandler {
  ...
  async handle(event: OutboxEvent) { const photoId = String(event.payload.photoId ?? event.aggregateId ?? ""); await this.uow.run(async ({repositories}) => { const [photo] = await repositories.db.select().from(productPhotos).where(eq(productPhotos.id, photoId)).limit(1); if (!photo || photo.processingStatus === ProductPhotoProcessingStatus.Ready) return; const storageKey = photo.storageKey ?? photo.filename; const thumbKey = photo.thumbnailStorageKey ?? `${photo.filename}-thumb`; await copyIdempotent(safeJoin(this.roots.tempRoot, storageKey), safeJoin(this.roots.permanentRoot, storageKey)); await copyIdempotent(safeJoin(this.roots.tempRoot, thumbKey), safeJoin(this.roots.permanentRoot, thumbKey)); await repositories.db.update(productPhotos).set({ url: `${this.roots.publicBase ?? "/images/product-photos"}/${storageKey}`, thumbnailUrl: `${this.roots.publicBase ?? "/images/product-photos"}/${thumbKey}`, storageKey, thumbnailStorageKey: thumbKey, processingStatus: ProductPhotoProcessingStatus.Ready, processingErrorCode: null, processingUpdatedAt: new Date().toISOString() }).where(eq(productPhotos.id, photoId)); await unlinkIdempotent(safeJoin(this.roots.tempRoot, storageKey)); await unlinkIdempotent(safeJoin(this.roots.tempRoot, thumbKey)); }); }
}
