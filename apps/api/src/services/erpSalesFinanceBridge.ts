...
/**
 * Sprint 80 correction: a fully reversed sale (saleReversals.financialsReversed=true) must
 * report an effective grossRevenue/taxVat/netRevenue/itemCost/profit of zero here - the original
 * sale_financials row itself is never touched (it remains the immutable, audit-preserved
 * completed-sale snapshot; see reverseCompletedSale). netRetainedRevenue/adjustedProfit are also
 * pinned to 0 rather than net-of-refund when reversed, since the full refund that made the
 * reversal eligible must not ALSO be subtracted from an already-zeroed net/profit figure
 * (double-counting the same economic event twice).
 */
export async function adjustedFinancials(db:DbClient, orderId:string){ const {o,items}=await orderCore(db,orderId); const [sf]=await db.select().from(saleFinancials).where(eq(saleFinancials.orderId,orderId)); const [reversal]=await db.select().from(saleReversals).where(and(eq(saleReversals.orderId,orderId),eq(saleReversals.financialsReversed,true))); const reversed=!!reversal; const [{totalRefunded}]=await db.select({totalRefunded:sql<number>`coalesce(sum(${refunds.totalAmount}),0)`}).from(refunds).where(and(eq(refunds.orderId,orderId),eq(refunds.status,"succeeded"))); const requiredKnown=!!sf && sf.shippingCost!=null && sf.itemCost!=null; const gross=reversed?0:(sf?.grossRevenue??o.totalAmount); const shippingCost=sf?.shippingCost??null; const itemCost=reversed?0:(sf?.itemCost??items.reduce((a,i)=>a,0)); const taxVat=reversed?0:(sf?.taxVat??o.taxAmount); const fees=[sf?.marketplaceFee,sf?.promotedFee,sf?.paymentFee]; const feesKnown=fees.every(v=>v!=null); const net=reversed?0:(sf?sf.netRevenue:money(o.totalAmount-o.taxAmount-(shippingCost??0)-(feesKnown?fees.reduce((a,v)=>a+Number(v),0):0))); const profit=reversed?0:(sf?.profit??null); return {currency:"EUR",grossRevenue:gross,shippingCharged:o.shippingAmount,discounts:0,taxVat,marketplaceFee:sf?.marketplaceFee??null,promotedFee:sf?.promotedFee??null,paymentFee:sf?.paymentFee??null,shippingCost,itemCost,netRevenue:requiredKnown?net:null,profit,completeness:requiredKnown?"Complete":"Incomplete",totalRefunded:totalRefunded??0,feeAdjustments:null,netRetainedRevenue:reversed?0:(requiredKnown?money(net-(totalRefunded??0)):null),adjustedProfit:reversed?0:(profit!=null?money(profit-(totalRefunded??0)):null),adjustedCompleteness:requiredKnown?"Complete":"Incomplete"}; }
...
/**
 * Sprint 80 correction: sales fully reversed (saleReversals.financialsReversed=true) contribute
 * zero to grossRevenue/itemCost/profit here, and their refunds are excluded from totalRefunds -
 * otherwise the reversal's refund would be subtracted a second time against an aggregate that no
 * longer includes that sale's revenue at all. The underlying sale_financials/refunds rows
 * themselves are never modified; this is a read-model-only correction.
 */
export async function financeSummary(db:DbClient,q:any={}){ const sales=await db.select().from(saleFinancials); const reversedOrderIds=new Set((await db.select().from(saleReversals).where(eq(saleReversals.financialsReversed,true))).map((r:any)=>r.orderId)); const effectiveSales=sales.map((s:any)=>reversedOrderIds.has(s.orderId)?{...s,grossRevenue:0,itemCost:0,profit:0}:s); const refs=(await db.select().from(refunds).where(eq(refunds.status,"succeeded"))).filter((r:any)=>!reversedOrderIds.has(r.orderId)); const gross=money(effectiveSales.reduce((a,s)=>a+s.grossRevenue,0)), totalRefunds=money(refs.reduce((a,r)=>a+r.totalAmount,0)); const itemCost=money(effectiveSales.reduce((a,s)=>a+s.itemCost,0)); const feesKnown=sales.every(s=>s.marketplaceFee!=null&&s.paymentFee!=null); const fees=feesKnown?money(sales.reduce((a,s)=>a+(s.marketplaceFee??0)+(s.promotedFee??0)+(s.paymentFee??0),0)):null; const shippingCost=sales.every(s=>s.shippingCost!=null)?money(sales.reduce((a,s)=>a+s.shippingCost,0)):null; const profit=sales.length?money(effectiveSales.reduce((a,s)=>a+s.profit,0)):null; return {currency:"EUR",grossRevenue:gross,totalRefunds,netRevenue:money(gross-totalRefunds),itemCost,fees,shippingCost,profit,adjustedProfit:profit==null?null:money(profit-totalRefunds),completeness:feesKnown?"Complete":"Incomplete"}; }
