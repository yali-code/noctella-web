...
export function createProductWriteTransactionCapabilityForDb<Driver extends ProductWriteTransactionDriver>(db: DbClient, driver: Driver): ProductWriteTransactionCapabilityFor<Driver> {
  if (driver === "sqlite" || driver === "test-memory") return { driver, execution: "synchronous", run(work) { let result: unknown; const transaction = db.transaction((tx) => { result = work({ repositories: { productWriteRepositories: { products: createSynchronousProductWriteRepositoryForDb(tx as any, driver) } } }); if (result instanceof Promise) throw new Error("SQLITE_ASYNC_PRODUCT_WRITE_TRANSACTION_CALLBACK_REJECTED"); }); if (typeof transaction === "function") (transaction as () => void)(); return result; } } as ProductWriteTransactionCapabilityFor<Driver>;
  return { driver, execution: "asynchronous", run: (work) => db.transaction(async (tx) => work({ repositories: { productWriteRepositories: { products: createProductWriteRepositoryBundleForDb(tx as any, driver).products } } })) } as ProductWriteTransactionCapabilityFor<Driver>;
}
