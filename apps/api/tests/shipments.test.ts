...
describe("Sprint 14 shipment workflow", () => {
  ...
  it("creates shipments idempotently and validates invalid order, Other carrier, EUR, and A1Post customs metadata", async () => { const s = await createShipment(db, { orderId: order.id, carrierCode: CarrierCode.A1Post, shippingCost: 12, customs: { productDescription: "Vase", hsCode: "6913", declaredValue: 100, weight: 2, ddpReference: "DDP", iossReference: "IOSS" } }); expect(s.carrierCode).toBe(CarrierCode.A1Post); expect((await createShipment(db, { orderId: order.id, carrierCode: CarrierCode.UPS })).id).toBe(s.id); await expect(createShipment(db, { orderId: "bad", carrierCode: CarrierCode.UPS })).rejects.toThrow(/Order not found/); await expect(createShipment(db, { orderId: order.id, carrierCode: CarrierCode.Other })).rejects.toThrow(/Custom carrier/); await cancelShipment(db, s.id); await expect(createShipment(db, { orderId: order.id, carrierCode: CarrierCode.Other })).rejects.toThrow(/Custom carrier/); await expect(createShipment(db, { orderId: order.id, carrierCode: CarrierCode.UPS, currency: "USD" })).rejects.toThrow(/EUR/); await expect(createShipment(db, { orderId: order.id, carrierCode: CarrierCode.UPS })).rejects.toThrow(/associated/); });
  it("requires shipment quantities to exactly match ReadyForShipment packing quantities", async () => { await expect(createShipment(db, { orderId: order.id, packingTaskId, carrierCode: CarrierCode.UPS, items: [{ orderItemId: order.items[0].id, quantity: 2 }] })).rejects.toThrow(/exactly match/); });
  ...
  describe("Sprint 40A: Shipment InTransit integrates with Order Shipped", () => {
    ...
    it("rejects first-time InTransit when the Order is already Shipped", async () => {
      const s = await createShipment(db, { orderId: order.id, carrierCode: CarrierCode.LocalPickup });
      await markReady(db, s.id);
      await db.update(schema.orders).set({ status: OrderStatus.Shipped }).where(eq(schema.orders.id, order.id));
      await expect(markShipped(db, s.id)).rejects.toThrow(/Processing/);
      expect((await getShipment(db, s.id)).status).toBe(ShipmentStatus.Ready);
      expect((await getOrderById(db, order.id)).status).toBe(OrderStatus.Shipped);
    });

    it("rejects InTransit atomically when the Order is Cancelled", async () => {
      const s = await createShipment(db, { orderId: order.id, carrierCode: CarrierCode.LocalPickup });
      await markReady(db, s.id);
    ...
    it("a failed atomic Order transition prevents marketplace fulfillment enqueue", async () => {
