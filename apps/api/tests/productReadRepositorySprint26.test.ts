...
describe("Sprint 26 executable SQLite Product read repositories", () => {
  ...
  test("title ascending uses Noctella Web names with canonical fallback and a stable id tie-breaker", async () => { const { db, repos } = await seedReadDb(); await db.update(sqliteSchema.products).set({ wooProductName:"Aardvark Display Name" }).where(eq(sqliteSchema.products.id,"p2")); expect((await repos.products.list({ sort:"title_asc", pageSize:10 })).map(p=>p.id)).toEqual(["p2","p1","p3"]); });
  test("title ascending uses canonical titles when Noctella Web names are absent", async () => { const { db, repos } = await seedReadDb(); await db.update(sqliteSchema.products).set({ wooProductName:null }).where(eq(sqliteSchema.products.id,"p2")); expect((await repos.products.list({ sort:"title_asc", pageSize:10 })).map(p=>p.id)).toEqual(["p1","p3","p2"]); });
  test("price sorting uses Noctella Web prices with canonical fallback", async () => { const { db, repos } = await seedReadDb(); await db.update(sqliteSchema.products).set({ wooListingPriceEur:40 }).where(eq(sqliteSchema.products.id,"p1")); await db.update(sqliteSchema.products).set({ wooListingPriceEur:5 }).where(eq(sqliteSchema.products.id,"p2")); expect((await repos.products.list({ sort:"price_asc", pageSize:10 })).map(p=>p.id)).toEqual(["p2","p3","p1"]); expect((await repos.products.list({ sort:"price_desc", pageSize:10 })).map(p=>p.id)).toEqual(["p1","p3","p2"]); });
