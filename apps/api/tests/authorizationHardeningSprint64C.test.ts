...
describe("Sprint 149 Archive safety HTTP contract",()=>{
  it("allows products.edit Archive preflight without granting lifecycle publish access",async()=>{const category=await createCategory(db,{name:`Archive preflight ${crypto.randomUUID()}`,displayOrder:0,isActive:true}),product=await createProduct(db,{sku:`PREFLIGHT-${crypto.randomUUID()}`,title:"Archive safe",type:ProductType.UniqueItem,status:ProductStatus.Draft,categoryId:category.id,stockQuantity:1,priceEur:10,customsWarning:false,isFeatured:false,allowMakeOffer:false,allowCashOnDelivery:false,showInArchiveAfterSale:false});const owner=await request(app).get(`/api/products/${product.id}/archive-safety`).set("Cookie",roleCookies.get(AdminRole.Owner)!);expect(owner.status).toBe(200);expect(owner.body).toEqual({canArchive:true});const editorCookie=roleCookies.get(AdminRole.ProductEditor)!;const editor=await request(app).get(`/api/products/${product.id}/archive-safety`).set("Cookie",editorCookie);expect(editor.status).toBe(200);expect(editor.body).toEqual({canArchive:true});expect((await request(app).get(`/api/products/${product.id}/lifecycle`).set("Cookie",editorCookie)).status).toBe(403);const forbiddenRole=Object.values(AdminRole).find(role=>!ROLE_PERMISSIONS[role].includes("products.edit"))!;expect((await request(app).get(`/api/products/${product.id}/archive-safety`).set("Cookie",roleCookies.get(forbiddenRole)!)).status).toBe(403);});

  it("returns sanitized unsafe preflight but independently rechecks Archive POST",async()=>{const category=await createCategory(db,{name:`Archive race ${crypto.randomUUID()}`,displayOrder:0,isActive:true}),product=await createProduct(db,{sku:`RACE-${crypto.randomUUID()}`,title:"Archive race",type:ProductType.UniqueItem,status:ProductStatus.Draft,categoryId:category.id,stockQuantity:1,priceEur:10,customsWarning:false,isFeatured:false,allowMakeOffer:false,allowCashOnDelivery:false,showInArchiveAfterSale:false}),cookie=roleCookies.get(AdminRole.Owner)!;expect((await request(app).get(`/api/products/${product.id}/archive-safety`).set("Cookie",cookie)).body).toEqual({canArchive:true});await db.insert(schema.marketplaceConnections).values({id:`race-connection-${product.id}`,channel:"etsy",accountLabel:"race",status:"connected",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});await db.insert(schema.externalListings).values({id:`race-listing-${product.id}`,productId:product.id,channel:"etsy",connectionId:`race-connection-${product.id}`,externalListingId:"must-not-leak",externalStatus:"active",payloadSnapshot:"{}",publishedAt:new Date().toISOString(),updatedAt:new Date().toISOString()});const preflight=await request(app).get(`/api/products/${product.id}/archive-safety`).set("Cookie",cookie);expect(preflight.status).toBe(200);expect(preflight.body).toEqual({canArchive:false,reason:"Product cannot be archived while sales shutdown is incomplete"});expect(JSON.stringify(preflight.body)).not.toContain("must-not-leak");expect((await request(app).post(`/api/products/${product.id}/archive`).set("Cookie",cookie)).status).toBe(409);expect((await db.select().from(schema.products).where(eq(schema.products.id,product.id)))[0].status).toBe(ProductStatus.Draft);});
