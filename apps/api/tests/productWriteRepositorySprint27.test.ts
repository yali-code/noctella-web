...
describe("Sprint 27 executable SQLite Product write repositories", () => {
  ...
  // Sprint 98: calls create() directly (bypassing any use-case-level existsBySku pre-check) to
  // prove the repository itself turns a genuine INSERT-time unique-constraint race (SKU or slug -
  // same code path, same classifier) into a structured created:false result instead of throwing.
  test("duplicate slug INSERT reports created:false instead of throwing", async()=>{ const db=createTestDb(); const r=createDrizzleProductWriteRepositories(db, sqliteSchema, "sqlite"); const values = { id:"p1", sku:"S1", title:"T", slug:"same-slug", type:ProductType.UniqueItem, status:ProductStatus.Draft, stockQuantity:1, priceEur:1, customsWarning:false, isFeatured:false, allowMakeOffer:false, allowCashOnDelivery:false, showInArchiveAfterSale:false, createdAt:"n", updatedAt:"n" }; const first = await r.products.create({ values }); expect(first.created).toBe(true); const second = await r.products.create({ values: { ...values, id:"p2", sku:"S2" } }); expect(second.created).toBe(false); });
