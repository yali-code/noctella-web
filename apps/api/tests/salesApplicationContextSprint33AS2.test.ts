...
describe("Sales application context Sprint 33A-S2", () => {
  const cases: Array<[string, () => void | Promise<void>]> = [
    ...
    ["factory rejects mismatched UnitOfWork driver", () => { const h = db(); const u = { driver: "postgres", run: (deps().unitOfWork as any).run } as never; try { expect(() => createSalesApplicationContextForDb({ db: h.db, driver: "sqlite", unitOfWork: u, logger: {}, clock: deps().clock, idGenerator: deps().idGenerator })).toThrow("SALES_TRANSACTION_DRIVER_MISMATCH:sqlite:postgres"); } finally { h.raw.close(); } }],
    ["factory accepts UnitOfWork with undefined driver", () => { const h = db(); const u = deps().unitOfWork; try { expect(createSalesApplicationContextForDb({ db: h.db, driver: "postgres", unitOfWork: u, logger: {}, clock: deps().clock, idGenerator: deps().idGenerator }).unitOfWork).toBe(u); } finally { h.raw.close(); } }],
