...
describe("ERP sales finance bridge", () => {
  ...
  it("completes sale once, posts one financial record/entry, does not move stock again, and preserves sale movements", async () => { const [p]=await seed(db,1); const order=await paidOrder(db,[p]); await shipment(db,order.id); const [draftInvoice]=(await listInvoices(db,{orderId:order.id})).items; await issueInvoice(db,(draftInvoice as any).id,{}); const beforeStock=(await db.select().from(schema.products).where(eq(schema.products.id,p.id)))[0].stockQuantity; const beforeMoves=(await db.select().from(schema.stockMovements)).length; const first=await completeSale(db,order.id); const second=await completeSale(db,order.id); expect(first.status).toBe(OrderStatus.Completed); expect(second.alreadyCompleted).toBe(true); expect((await db.select().from(schema.products).where(eq(schema.products.id,p.id)))[0].stockQuantity).toBe(beforeStock); expect((await db.select().from(schema.stockMovements))).toHaveLength(beforeMoves); expect((await db.select().from(schema.saleFinancials).where(eq(schema.saleFinancials.orderId,order.id)))).toHaveLength(1); expect((await listFinanceEntries(db,{entryType:"CompleteSale"})).items).toHaveLength(1); });
